...
module Infer = struct
  ...
  let rec infer_expression env = function
    ...
    | Exp_apply (exp1, exp2) ->
      (match exp1 with
       | Exp_ident opr when is_unary_minus opr ->
  ...
  let infer_structure_item ~debug (env, out_list) =
    let get_names_from_let_binds env =
      RList.fold_left ~init:(return []) ~f:(fun acc { pat; _ } ->
        extract_names_from_pat
          (fun acc id ->
             return (List.rev_append [ Some id, TypeEnv.find_type_exn env id ] acc))
          acc
          pat)
    in
    function
    ...
    | Struct_value (Nonrecursive, value_binding, value_binding_list) ->
      ...
      let* id_list_rev = get_names_from_let_binds env value_binding_list in
      let id_list = List.rev id_list_rev in
      if debug then TypeEnv.pp Format.std_formatter env;
      return (env, List.rev_append id_list out_list)
...
let bin_op_scheme =
  Scheme (VarSet.empty, Type_arrow (Type_int, Type_arrow (Type_int, Type_int)))
;;

let comp_op_scheme =
  Scheme
    (VarSet.singleton "a", Type_arrow (Type_var "a", Type_arrow (Type_var "a", Type_bool)))
;;
